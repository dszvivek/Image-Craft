...
export const HeroDropZone: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto px-4 my-6">
      ...
      {!selectedFile ? (
        // IDLE STATE: High-Aesthetic Interactive Drop Surface
        <div
          ...
          className={`relative group rounded-3xl p-6 sm:p-8 border-2 border-dashed transition-all duration-300 cursor-pointer overflow-hidden ${
            isDragActive
              ? 'border-indigo-500 bg-indigo-50/80 dark:bg-indigo-950/40 shadow-2xl shadow-indigo-500/20 scale-[1.01]'
              : 'border-slate-300/90 dark:border-slate-700/80 hover:border-indigo-400 dark:hover:border-indigo-500/80 bg-white/80 dark:bg-slate-900/60 hover:bg-indigo-50/20 dark:hover:bg-slate-900/80 shadow-lg shadow-slate-200/40 dark:shadow-none'
          }`}
        >
          {/* Subtle Ambient Glow */}
          <div className="absolute -right-16 -top-16 w-44 h-44 bg-indigo-500/10 rounded-full blur-3xl group-hover:bg-indigo-500/20 transition-all pointer-events-none" />
          <div className="absolute -left-16 -bottom-16 w-44 h-44 bg-purple-500/10 rounded-full blur-3xl group-hover:bg-purple-500/20 transition-all pointer-events-none" />

          <div className="relative z-10 flex flex-col items-center text-center">
            {/* Animated Icon Ring */}
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-600 text-white flex items-center justify-center mb-4 shadow-xl shadow-indigo-500/25 group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300">
              <Upload className="w-7 h-7 animate-pulse-slow" />
            </div>

            <h3 className="text-lg sm:text-xl font-extrabold text-slate-900 dark:text-slate-100 tracking-tight mb-1.5">
              Drop any image here to edit instantly
            </h3>
            
            <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 max-w-md mb-4 font-medium leading-relaxed">
              Drag & drop, paste (<kbd className="font-mono text-[11px] px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 font-bold">Ctrl+V</kbd>), or click to browse.
            </p>

            {/* Format Badges */}
            <div className="flex flex-wrap items-center justify-center gap-1.5 mb-5 text-[10px] font-bold text-slate-500 dark:text-slate-400">
              <span className="px-2.5 py-0.8 rounded-md bg-slate-100/90 dark:bg-slate-800/80 border border-slate-200/60 dark:border-slate-700">PNG</span>
              <span className="px-2.5 py-0.8 rounded-md bg-slate-100/90 dark:bg-slate-800/80 border border-slate-200/60 dark:border-slate-700">JPG</span>
              <span className="px-2.5 py-0.8 rounded-md bg-slate-100/90 dark:bg-slate-800/80 border border-slate-200/60 dark:border-slate-700">WEBP</span>
              <span className="px-2.5 py-0.8 rounded-md bg-slate-100/90 dark:bg-slate-800/80 border border-slate-200/60 dark:border-slate-700">SVG</span>
              <span className="px-2.5 py-0.8 rounded-md bg-slate-100/90 dark:bg-slate-800/80 border border-slate-200/60 dark:border-slate-700">PDF</span>
              <span className="px-2.5 py-0.8 rounded-md bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-400 border border-emerald-200/60 dark:border-emerald-800/60">
                Up to 100MB
              </span>
            </div>

            {/* Quick Sample Action */}
            <div className="pt-2 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-center gap-3">
              <button
                type="button"
                onClick={(e) => {
                  e.stopPropagation();
                ...
                className="text-xs font-bold text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 flex items-center gap-1.5 px-3 py-1.5 rounded-xl hover:bg-indigo-50 dark:hover:bg-indigo-950/50 transition-colors cursor-pointer"
              >
