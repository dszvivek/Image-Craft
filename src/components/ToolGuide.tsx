  ...
  return (
    <div className="mt-16 border-t border-slate-200/60 dark:border-slate-800 pt-12 max-w-4xl mx-auto space-y-12 text-left">
      ...
      <div className="text-center space-y-3">
        ...
        <p className="text-xs md:text-sm text-slate-500 dark:text-slate-400 max-w-2xl mx-auto leading-relaxed font-medium">
          {introText}
        </p>
      </div>
          ...
          <ul className="space-y-3.5">
            {features.map((feature, idx) => (
              <li key={idx} className="flex items-start gap-2.5">
                ...
                <span className="text-[11px] md:text-xs font-medium text-slate-650 dark:text-slate-300 leading-relaxed">
                  {feature}
                </span>
              </li>
            ))}
            ...
            {faq.map((item, index) => (
              <div key={index} className="py-3.5 first:pt-0 last:pb-0">
                ...
                >
                  ...
                  {openFaqIndex === index ? (
                    <ChevronUp className="w-4 h-4 text-indigo-650 dark:text-indigo-400 shrink-0" />
                  ) : (
