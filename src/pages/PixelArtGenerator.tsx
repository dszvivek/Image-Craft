...
import { Download, RefreshCw, Gamepad2, Grid, Eye } from 'lucide-react';
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              ...
              <div className="premium-bento p-5 sm:p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 space-y-5 shadow-xl shadow-slate-200/20 dark:shadow-none">
                ...
                <div className="space-y-2 pb-3 border-b border-slate-100 dark:border-slate-800">
                  ...
                  <div className="grid grid-cols-3 gap-1.5">
                    {[
                      { label: '⚡ 8-Bit Retro', size: 16, pal: 'gameboy', dither: 'floyd-steinberg' },
                      { label: '✨ 16-Bit Arcade', size: 8, pal: 'pico8', dither: 'bayer' },
                      { label: '🎨 Fine Synth', size: 4, pal: 'cyberpunk', dither: 'none' },
                    ].map((preset, idx) => (
                      <button
                        key={idx}
                        onClick={() => {
                          setPixelSize(preset.size);
                          setSelectedPaletteId(preset.pal);
                          setDitherMode(preset.dither as any);
                        }}
                        className="py-2 px-1.5 rounded-xl text-[10px] font-bold bg-slate-100 dark:bg-slate-800 hover:bg-fuchsia-50 dark:hover:bg-fuchsia-950/40 text-slate-700 dark:text-slate-300 hover:text-fuchsia-600 border border-slate-200/60 dark:border-slate-700/60 transition cursor-pointer active:scale-95 text-center truncate"
                      >
                        {preset.label}
                      </button>
                    ))}
                ...
                {/* Pixel Block Size with Touch Steppers */}
                <div className="space-y-2">
                  <div className="flex justify-between items-center text-xs font-bold text-slate-600 dark:text-slate-400">
                    ...
                    <div className="flex items-center gap-1.5 font-mono text-fuchsia-600">
                      <button 
                        onClick={() => setPixelSize(Math.max(2, pixelSize - 2))}
                      ...
                      <span className="min-w-[36px] text-center font-bold">{pixelSize} px</span>
                      <button 
                        onClick={() => setPixelSize(Math.min(48, pixelSize + 2))}
                        className="w-5 h-5 rounded bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 flex items-center justify-center text-xs font-black cursor-pointer active:scale-90 transition"
                        title="Larger pixel blocks"
                      >
                        +
                      </button>
                    </div>
                  ...
                  <input
                    ...
                    className="range-styled w-full accent-fuchsia-600"
                  />
                      ...
                      <button
                        ...
                        className={`w-full p-2.5 rounded-xl border text-left transition-all cursor-pointer flex items-center justify-between gap-3 active:scale-98 ${
                          selectedPaletteId === pal.id
                            ? 'bg-fuchsia-50 dark:bg-fuchsia-950/40 border-fuchsia-400 dark:border-fuchsia-600 shadow-sm ring-1 ring-fuchsia-500/20'
                  ...
                  <div className="grid grid-cols-3 gap-1.5">
                    {(
                      ...
                      <button
                        ...
                        className={`py-2 px-1 rounded-xl text-xs font-bold transition-all cursor-pointer text-center active:scale-95 ${
                          ditherMode === m.id
                ...
                <div className="space-y-3 pt-2 border-t border-slate-100 dark:border-slate-800">
                  <div className="space-y-1">
                    <div className="flex justify-between items-center text-xs font-bold text-slate-600 dark:text-slate-400">
                      ...
                      <div className="flex items-center gap-1.5 font-mono text-fuchsia-600">
                        <button 
                          onClick={() => setContrastBoost(Math.max(-30, contrastBoost - 5))}
                          className="w-5 h-5 rounded bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 flex items-center justify-center text-xs font-black cursor-pointer active:scale-90 transition"
                        >
                          -
                        </button>
                        <span className="min-w-[32px] text-center">{contrastBoost > 0 ? `+${contrastBoost}` : contrastBoost}</span>
                        <button 
                          onClick={() => setContrastBoost(Math.min(40, contrastBoost + 5))}
                          className="w-5 h-5 rounded bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 flex items-center justify-center text-xs font-black cursor-pointer active:scale-90 transition"
                        >
                          +
                        </button>
                  ...
                  <div className="flex items-center justify-between pt-1">
                    ...
                    <button
                      ...
                      className={`px-3 py-1 rounded-xl text-xs font-bold border transition-all cursor-pointer active:scale-95 ${
                        showGrid
                          ? 'bg-fuchsia-50 dark:bg-fuchsia-950/50 border-fuchsia-400 text-fuchsia-600 dark:text-fuchsia-400 shadow-xs'
                    ...
                    <div className="flex gap-1.5">
                      {(['png', 'svg', 'webp'] as const).map((fmt) => (
                        <button
                          ...
                          className={`px-2.5 py-1 rounded-lg text-xs font-bold uppercase transition-all cursor-pointer active:scale-95 ${
                            exportFormat === fmt
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button
                    ...
                    className="flex-2 py-3 px-4 rounded-2xl bg-gradient-to-r from-fuchsia-600 to-pink-600 hover:from-fuchsia-700 hover:to-pink-700 text-white font-bold text-xs shadow-lg shadow-fuchsia-500/20 transition-all cursor-pointer flex items-center justify-center gap-2 disabled:opacity-50 active:scale-95"
                  >
            ...
            <div ref={previewContainerRef} className="lg:col-span-7 space-y-4 order-1 lg:order-2">
              <div className="relative rounded-3xl bg-slate-950/5 dark:bg-slate-950/50 border border-slate-200/80 dark:border-slate-800 p-4 min-h-[420px] flex items-center justify-center overflow-hidden">
                ...
                {/* Hold to Compare Overlay or Pixel Canvas */}
                {isComparing ? (
        ...
        {imageUrl && (
          <div className="fixed bottom-4 left-4 right-4 sm:hidden z-30 flex items-center justify-between p-2.5 bg-slate-900/90 dark:bg-slate-850/95 text-white rounded-2xl backdrop-blur-xl shadow-2xl border border-white/10 animate-fade-in">
            ...
            <button
              onClick={handleDownload}
              disabled={isProcessing}
              className="py-2 px-4 bg-gradient-to-r from-fuchsia-600 to-pink-600 hover:from-fuchsia-700 text-white rounded-xl text-xs font-bold shadow-md flex items-center gap-1.5 cursor-pointer active:scale-95 transition"
            >
              <Download className="w-4 h-4" />
