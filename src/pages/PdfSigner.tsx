...
export const PdfSigner: React.FC = () => {
  ...
  }, [pdfBytes, currentPage, zoomScale, renderPage]);
  ...
  const generateBase64FromType = useCallback(() => {
    if (!typedName.trim()) {
      setActiveSignature('');
      return;
    }
  ...
  }, [typedName, typedFont, drawColor, signatureMode, generateBase64FromType]);
  ...
  const handleContainerMouseMove = (e: React.MouseEvent<HTMLDivElement> | React.TouchEvent<HTMLDivElement>) => {
    ...
    if (isResizingRef.current && resizeStartRef.current) {
      ...
      const newX = Math.max(0, Math.min(100 - (currentSigWidth / canvas.width) * 100, placement.x + xChangePercent));
      const newY = Math.max(0, Math.min(100 - (currentSigHeight / canvas.height) * 100, placement.y + yChangePercent));
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <ToolGuide
          ...
          competitorComparison={{
            ...
            benefit: 'Commercial PDF signing suites route your documents through cloud servers and cap monthly document exports on free tiers. ImagePlumber PDF Signer embeds signatures entirely inside your local browser memory. It is 100% free, imposes no document count limits, and operates with zero cloud transmission.'
          }}
          steps={[
            {
