...
export function detectSmartCrop(img: HTMLImageElement, targetAspect: number): SmartCropResult {
  ...
  if (faceAreaRatio > 0.015 && faceAreaRatio < 0.35) {
    ...
    const left = Math.max(0, fMinX - paddingX) / faceW;
    const top = Math.max(0, fMinY - paddingY * 1.5) / faceH; // More space for forehead
    const right = Math.min(faceW, fMaxX + paddingX) / faceW;
    const bottom = Math.min(faceH, fMaxY + paddingY) / faceH;
    const w = right - left;
    const h = bottom - top;
  ...
  const threshSal = totalSal * 0.08 / (salW * salH); // Threshold filter
